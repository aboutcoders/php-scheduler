...
class LimitMemoryExtension implements PreProcessScheduleExtensionInterface, ScheduleProcessedExtensionInterface
{
    /**
     * @var int
     */
    ...
    protected function shouldBeStopped(LoggerInterface $logger): bool
    {
        ...
        if ($memoryUsage >= $this->memoryLimit) {
            $logger->debug(sprintf(
                '[LimitMemoryExtension] Interrupt execution as memory limit reached. limit: "%s", used: "%s"',
                $this->memoryLimit,
                $memoryUsage
            ));
