...
class CheckCronExpressionExtension implements CheckScheduleExtensionInterface
{
    ...
    public function onCheckSchedule(CheckSchedule $context): void
    {
        ...
        if (! CronExpression::isValidExpression($expression)) {
            $context->getLogger()->debug(sprintf('[CheckCronExpressionsExtension] Skip invalid cron "%s"', $expression));
            return;
        }
        ...
        $context->getLogger()->debug(sprintf('[CheckCronExpressionsExtension] Schedule with expression "%s" is due', $expression));
